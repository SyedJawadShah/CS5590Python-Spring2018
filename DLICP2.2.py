...
X1 = tf.placeholder(tf.float32, name='Smokingstatus')
X2 = tf.placeholder(tf.float32, name='Ageclassification')
Y = tf.placeholder(tf.float32, name='Deathstatus')

# Step 3: create weight and bias, initialized to 0
w1 = tf.Variable(0.0, name='weights')
w2 = tf.Variable(1.1, name='weights2')
...
with tf.Session() as sess:
    ...
    for i in range(50):  # train the model 50 epochs
        ...
        for e, f,g,h in data:
            ...
            _, l = sess.run([optimizer, loss], feed_dict={X1: e, X2:f, Y: g})
            total_loss += l
